    ...
    public static function getattendance(Request $request)
    {
        ...
        try {
            ...
            foreach ($logdata as $k => $v) {
                $newpin = '';
                if (stripos($machine->company, 'SINAR TERANG') !== false) {
                    if (substr($v['pin'], 0, 2) !== 80) {
                        $newpin = "80{$v['pin']}";
                    } else {
                        $newpin = $v['pin'];
                    }
                ...
                if ($employeeData) {
                    ...
                    $check = Attendance::where(['karyawan_id' => $newpin, 'tgl_absen' => $dataInsert['tgl_absen'], 'status' => self::$desc[$v['status_scan']]])->count();
