...
class Employee extends Model
{
    ...
    public static function getDataEmployees(Request $request, $data = null)
    {
        ...
        try {
            ...
            $requestEmployee = Http::withToken(self::$JWTTOKEN)->post(self::$getEmployeeUrl, $dataSend);
            $requestEmployee = $requestEmployee->json();
            $employeesData['rows'] = $employeesData['rows'] + $requestEmployee['rows'];

            $dataSend['company'] = 'SINAR TERANG';
            $requestEmployeeSinter = Http::withToken(self::$JWTTOKEN)->post(self::$getEmployeeUrl, $dataSend);
            $requestEmployeeSinter = $requestEmployeeSinter->json();
            $employeesData['rows'] = $employeesData['rows'] + $requestEmployeeSinter['rows'];
            ...
            foreach ($requestEmployee['data'] as $k => &$v) {
                ...
                array_push($employeesData['data'], $v);
            }

            foreach ($requestEmployeeSinter['data'] as $k => &$v) {
